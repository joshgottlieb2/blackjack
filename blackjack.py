...
def deal_card():
    ...
    global dealt_card, dealt_card_value, player_value, dealer_value, current_bet, player_cash, account_value
    dealt_card = (random.choice(cards['card']))
    idx = cards['card'].index(dealt_card)
    dealt_card_value = cards['value'][idx]
    

def choice():

    global dealt_card, dealt_card_value, player_value, dealer_value, current_bet, player_cash, account_value
    selection = input("What do you want to do? Type: 'Hit', 'Stay', or 'Double' ").lower()
    if selection == 'hit':
        deal_card()
...
def play_again():
    ...
    again = input("Play again? Type 'yes' to play again or 'no' to leave the table.").lower()
    if again == 'yes':
        player_begin()
    else:
    ...
    while dealer_done == False:
        ...
        if dealer_value > 16:
            ...
            if dealt_card_value == x:
                if dealer_value < 11:
                    dealer_value += 11
                    print(f"Dealer takes a card. Dealer gets an ace. Dealer has {dealer_value}.")
                else:
                    dealer_value += 1
                    print(f"Dealer takes a card. Dealer gets an ace. Dealer has {dealer_value}.")
            else:
                dealer_value += dealt_card_value
                print(f"Dealer takes a card. Dealer gets a {dealt_card_value}. Dealer has {dealer_value}.")
